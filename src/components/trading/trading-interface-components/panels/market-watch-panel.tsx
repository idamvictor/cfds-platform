...
const MarketWatchPanel = ({ addCurrencyPair }: MarketWatchPanelProps) => {
  ...
  const error = useAssetStore((state) => state.error);
    ...
    const { subscribeToAll, subscribeToCategory } = useAssetWebSocket({
        onConnected: () => {
            subscribeToAll();
        }
    });
    ...
    useEffect(() => {
        ...
        if (categories.length > 0 && expandedCategories.length === 0) {
            ...
            // Subscribe to each available category
            categories.forEach(category => {
                subscribeToCategory(category);
            });
        }
    }, [groupedAssets, expandedCategories.length, subscribeToCategory]);
    ...
    }, [expandedCategories, subscribeToCategory]);
