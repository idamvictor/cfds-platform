...
const Sidebar: React.FC<SidebarProps> = ({ onLinkClick }) => {
  ...
  const baseNavItems = [
    ...
    { title: "Transaction History", icon: History, path: "/main/transaction-history" },
  ...
  const navItems = React.useMemo(() => {
    ...
    if (settings?.is_mt4) {
      const accountsIndex = items.findIndex(
        (item) => item.path === "/main/accounts",
      );
      const insertIndex =
        accountsIndex !== -1 ? accountsIndex + 1 : items.length - 2;
