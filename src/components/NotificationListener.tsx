...
export function NotificationListener() {
    ...
    useEffect(() => {
        if (!isReadingNotification && !activeNotification && user?.notifications && user.notifications.length > 0) {
            // Find the first unread notification
            const firstUnread = user.notifications.find(notification => !notification.read);

            // Set it as active if it exists and is different from the last seen notification
            if (firstUnread && (!lastNotificationIdRef.current || firstUnread.id !== lastNotificationIdRef.current)) {
    ...
    const handleNotification = useCallback((data: NotificationData) => {
        ...
        if (isReadingNotification) {
            ...
            pendingNotificationsRef.current.push(newNotification as UserNotification);
